...
## บันทึกรูปผลการทดลอง
<img width="1918" height="987" alt="image" src="https://github.com/user-attachments/assets/e1b3e4a0-34a4-480b-8561-acde70bba0e1" />
...
<img width="1918" height="917" alt="image" src="https://github.com/user-attachments/assets/10708a77-dba7-4761-a630-e54be54f3b28" />
...
<img width="1918" height="917" alt="image" src="https://github.com/user-attachments/assets/bef81868-f9d9-471f-9f9c-8aacf4888d5f" />
...
- GitHub Actions ทำหน้าที่เป็น **ระบบอัตโนมัติ (CI/CD)** สำหรับตรวจสอบและเผยแพร่โค้ดของโปรเจกต์ โดยในงานนี้จะทำงานทุกครั้งที่มีการ `push` หรือ `pull request` ไปยัง branch `main` ซึ่งจะช่วยให้กระบวนการ Deploy เว็บไซต์เกิดขึ้นโดยอัตโนมัติ
ขั้นตอนหลักของ Workflow ประกอบด้วย:
- 1. **Checkout code** — ดึงซอร์สโค้ดจาก GitHub repository มาใช้ใน runner ของ GitHub Actions  
- 2. **Setup Node.js** — ติดตั้ง Node.js เวอร์ชันที่กำหนด (เช่น 18.x) เพื่อให้สามารถใช้คำสั่ง `npm` และ `next` ได้  
- 3. **Install dependencies** — ติดตั้ง library ที่จำเป็นทั้งหมดจากไฟล์ `package.json` เช่น `next`, `react`, `axios`, `tailwindcss` เป็นต้น  
- 4. **Lint** — ตรวจสอบความถูกต้องของโค้ดด้วย ESLint เพื่อป้องกัน syntax error และเพิ่มคุณภาพของโค้ด  
- 5. **Build Next.js app** — ใช้คำสั่ง `npm run build` เพื่อสร้างไฟล์ Static Export ของ Next.js ที่พร้อมสำหรับการ deploy (ในโฟลเดอร์ `out/`)  
- 6. **Upload artifact** — อัปโหลดไฟล์ผลลัพธ์ (artifact) เพื่อส่งต่อให้ขั้นตอน Deploy  
- 7. **Deploy to GitHub Pages** — นำไฟล์ที่ build แล้วจากโฟลเดอร์ `out/` ไปเผยแพร่บน GitHub Pages อัตโนมัติ  
...
- CORS (Cross-Origin Resource Sharing) คือกลไกความปลอดภัยของเว็บเบราว์เซอร์ที่ป้องกันไม่ให้เว็บไซต์หนึ่งเข้าถึงข้อมูลจากอีกโดเมนหนึ่งโดยไม่ได้รับอนุญาต ตัวอย่างเช่น

- Frontend อยู่ที่: `https://rattapum1269.github.io/todo-frontend/`  
- Backend อยู่ที่: `https://flask-todo-cicd.onrender.com`

 - เมื่อ Frontend เรียก API ที่อยู่บน Backend จะถือว่าเป็น **cross-origin request** ซึ่งเบราว์เซอร์จะปฏิเสธทันทีหาก Backend ไม่อนุญาตไว้ล่วงหน้า
ดังนั้นในฝั่ง Flask (Backend) จึงต้อง **เปิดใช้งาน CORS** 
